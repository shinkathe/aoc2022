...
printfn "Answer 1: %i" (intermediate |> Array.sumBy (fun (_, size) -> if size <= 100000 then size else 0))

let limit = intermediate |> Array.find (fun (a, _) -> a = "base") |> (snd >> (+) 30000000 >> (+) -70000000)
printfn "Answer 2: %A" (intermediate |> Array.where (snd >> (<=) limit) |> Array.sortBy (snd) |> Array.head)