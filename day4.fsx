...
let setsAnyOverlap = fun t -> (t ||> Set.intersect) |> Seq.isEmpty |> not
let input =
    System.IO.File.ReadAllLines "inputs/day4.txt"
    |> Array.map ((fun s -> s.Split(',', '-')) >> Array.map int >> toSet)

input |> Array.countBy isFullyContained |> toPair |> snd |> snd |> printfn "Answer 1: %A"
input |> Array.countBy setsAnyOverlap |> toPair |> fst |> snd |> printfn "Answer 2: %A"
